...
class Auth extends BaseController
{
	...
	public function logout()
	{
		...
		$this->session->setFlashdata('message', $this->ionAuth->messages());
		return redirect()->to('/auth2/login');
	}
	...
	public function change_password()
	{
		...
		else
		{
			...
			{
				$this->session->setFlashdata('message', $this->ionAuth->errors($this->validationListTemplate));
				return redirect()->to('/auth2/change_password');
			}
		}
	}
	...
	public function forgot_password()
	{
		...
		else
		{
			...
			if (empty($identity))
			{
				...
				$this->session->setFlashdata('message', $this->ionAuth->errors($this->validationListTemplate));
				return redirect()->to('/auth2/forgot_password');
			}
			...
			if ($forgotten)
			{
				...
				$this->session->setFlashdata('message', $this->ionAuth->messages());
				return redirect()->to('/auth2/login'); //we should display a confirmation page here instead of the login page
			}
			else
			{
				$this->session->setFlashdata('message', $this->ionAuth->errors($this->validationListTemplate));
		...
		if ($user)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						$this->session->setFlashdata('message', $this->ionAuth->errors($this->validationListTemplate));
						return redirect()->to('/auth2/reset_password/' . $code);
					}
				}
			}
		}
	...
	public function activate(int $id, string $code = ''): \CodeIgniter\HTTP\RedirectResponse
	{
		...
		if ($activation)
		{
			// redirect them to the auth2 page
			$this->session->setFlashdata('message', $this->ionAuth->messages());
			return redirect()->to('/auth2');
		}
		else
		{
	...
	public function deactivate(int $id = 0)
	{
		...
		else
		{
			...
			// redirect them back to the auth2 page
			return redirect()->to('/auth2');
		}
	}
